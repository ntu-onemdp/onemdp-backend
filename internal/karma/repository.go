...
func (r *karmaRepository) insert(semester string) error {
	query := fmt.Sprintf(`INSERT INTO %s (SEMESTER) VALUES ($1);`)
...
func (r *karmaRepository) update(settings Karma) error {
	query := fmt.Sprintf(`UPDATE %s SET CREATE_THREAD=$1, CREATE_ARTICLE=$2, CREATE_COMMENT=$3, CREATE_POST=$4, LIKE=$5 WHERE SEMESTER = (SELECT SEMESTER FROM %s WHERE IS_CURRENT);`, KARMA_TABLE, semester.SEMESTER_TABLE)
